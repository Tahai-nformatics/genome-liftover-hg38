...
usage ./liftover_hg19.sh /path/to/dataset/example_dataset[without .bed/.bim/.fam extension] example_dataset /path/to/reference_files
"""
filepath=$1
datasetname=$2
referencefile_path=$3
...
plink2 --bfile $filepath --fa $referencefile_path/hg19.fa --ref-from-fa force --rm-dup force-first --make-bed --out $PWD/$datasetname/$dataset/genotypes_fa
...
#3z Convert XY chr to X chr - will be used for chr:position match as snp151.txt does not have chr XY
awk 'BaGIN{FS=OFS="\t"} $1=="XY" {$1="X"} {print}' $PWD/$datasetname/$dataset/updated_genotypes_fa_gsadups_syntaxupdate3_rm_dups.bim > $PWD/$datasetname/$dataset/updated2_genotypes_fa.bim
...
#3b Add Chr:Pos to files for Chr:pos findings
...
if [ ! -f "$PWD/$datasetname/$dataset/merged_rsids_chrpos_genotypes_snp151.txt" ]; then
    LC_ALL=C join -t$'\t' -1 2 -2 5 -a 1 -e "EMPTY" -o 1.{1..7} 2.{1..27}  $PWD/$datasetname/$dataset/sorted_genotypes_fa_rsids_chrpos.bim $referencefile_path/snp151_chr_pos_rsidsorted.txt >  $PWD/$datasetname/$dataset/merged_rsids_chrpos_genotypes_snp151.txt
fi

if [ ! -f "$PWD/$datasetname/$dataset/merged_nonrsids_chrpos_genotypes_snp151.txt" ]; then
    LC_ALL=C join -t$'\t' -1 7 -2 27 -a 1 -e "EMPTY" -o 1.{1..7} 2.{1..27}  $PWD/$datasetname/$dataset/sorted_genotypes_fa_norsids_chrpos.bim $referencefile_path/snp151_chr_pos_norsidsorted.txt >  $PWD/$datasetname/$dataset/merged_nonrsids_chrpos_genotypes_snp151.txt
fi
...
plink1.9 --bfile $PWD/$datasetname/$dataset/updated2_genotypes_fa_mapped1 --keep-allele-order --exclude $PWD/$datasetname/$dataset/exclusion_list.txt --make-bed --out $PWD/$datasetname/$dataset/genotypes_fa_flipped_fa_droppedempty
...
plink1.9 --bfile $PWD/$datasetname/$dataset/genotypes_fa_flipped_fa_droppedempty --keep-allele-order --exclude $PWD/$datasetname/$dataset/exclusion_list2.txt --make-bed --out $PWD/$datasetname/$dataset/genotypes_fa_flipped_fa_droppedempty_nodups
...
plink1.9 --bfile $PWD/$datasetname/$dataset/genotypes_fa_flipped_fa_droppedempty_nodups_v3_nodups --flip $PWD/$datasetname/$dataset/rsids_to_be_flipped_uniq.txt --make-bed --out $PWD/$datasetname/$dataset/updated_genotypes_fa_mapped_flipped
## 9b set A2 to genomic REF again (now for the variants that were complemented/'flipped')
plink2 --bfile $PWD/$datasetname/$dataset/updated_genotypes_fa_mapped_flipped --fa $referencefile_path/hg19.fa --ref-from-fa force --make-bed --out $PWD/$datasetname/$dataset/genotypes_fa_flipped_fa
...
plink1.9 --bfile $PWD/$datasetname/$dataset/genotypes_fa_flipped_fa --keep-allele-order --update-chr $PWD/$datasetname/$dataset/mapping_chr.txt --update-map $PWD/$datasetname/$dataset/mapping_position.txt --make-bed --out $PWD/$datasetname/$dataset/genotypes_fa_flipped_mapped_fa
...
/home/iqbalt/tools/liftOver -bedPlus=3 -tab $PWD/$datasetname/$dataset/genotypes_fa_flipped_fa_dropped.bim.hg18.bed $referencefile_path/hg19ToHg38.over.chain.gz $PWD/$datasetname/$dataset/genotypes_fa_flipped_fa_dropped.bim.hg18.lifted_to_hg38.bed $PWD/$datasetname/$dataset/genotypes_fa_flipped_fa_dropped.bim.hg18.unlifted.bed
...
samtools faidx $referencefile_path/hg38.fa -r $PWD/$datasetname/$dataset/genotypes_fa_flipped_fa_dropped.bim.hg18.lifted_to_hg38.bed.regions | grep -v "^>" > $PWD/$datasetname/$dataset/genotypes_fa_flipped_fa_dropped.bim.hg18.lifted_to_hg38.bed.regions.ref_alleles
...
plink1.9 --bfile $PWD/$datasetname/$dataset/genotypes_fa_flipped_mapped_fa --keep-allele-order --exclude $PWD/$datasetname/$dataset/genotypes_fa_flipped_fa_dropped.bim.to_be_dropped_after_liftover --make-bed --out $PWD/$datasetname/$dataset/genotypes_fa_flipped_fa_dropped_droppedAfterLiftover
### 8.8. make a new/updated hg38 plink fileset
plink1.9 --bfile $PWD/$datasetname/$dataset/genotypes_fa_flipped_fa_dropped_droppedAfterLiftover --update-map $PWD/$datasetname/$dataset/genotypes_fa_flipped_fa_dropped.bim.hg18.lifted_to_hg38.with_genomic_ref.matching_a2_and_ref.bed.updated_positions --make-bed --keep-allele-order --out $PWD/$datasetname/$dataset/genotypes_fa_flipped_fa_dropped_droppedAfterLiftover_hg38
##Convert chrX to chr23, chrY to 24, ChrXY to 25 as per PLINK standards

plink1.9 --bfile $PWD/$datasetname/$dataset/genotypes_fa_flipped_fa_dropped_droppedAfterLiftover_hg38 --make-bed --keep-allele-order --out $PWD/$datasetname/$dataset/$datasetname.NHW.fwd.hg38.qc
