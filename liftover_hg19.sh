...
plink2 --bfile $filepath --fa $PWD/scripts/hg19.fa --ref-from-fa force --rm-dup force-first --make-bed --out $PWD/$datasetname/$dataset/genotypes_fa
...
awk 'BEGIN{FS=OFS="\t"} $1=="XY" {$1="X"} {print}' $PWD/$datasetname/$dataset/updated_genotypes_fa_gsadups_syntaxupdate2_rm_dups.bim > $PWD/$datasetname/$dataset/updated2_genotypes_fa.bim
awk 'BEGIN{FS=OFS="\t"} $1=="XY" {$1="X"} {print}' $PWD/$datasetname/$dataset/updated_genotypes_gsadups_removed_syntaxupdate2_rm_dups.bim > $PWD/$datasetname/$dataset/updated2_genotypes.bim
...
if [ ! -f "$PWD/$datasetname/$dataset/merged_rsids_chrpos_genotypes_snp151.txt" ]; then
    LC_ALL=C join -t$'\t' -1 2 -2 5 -a 1 -e "EMPTY" -o 1.{1..7} 2.{1..27}  $PWD/$datasetname/$dataset/sorted_genotypes_fa_rsids_chrpos.bim $PWD/scripts/snp151_chr_pos_rsidsorted4.txt >  $PWD/$datasetname/$dataset/merged_rsids_chrpos_genotypes_snp151.txt
fi

if [ ! -f "$PWD/$datasetname/$dataset/merged_nonrsids_chrpos_genotypes_snp151.txt" ]; then
    LC_ALL=C join -t$'\t' -1 7 -2 27 -a 1 -e "EMPTY" -o 1.{1..7} 2.{1..27}  $PWD/$datasetname/$dataset/sorted_genotypes_fa_norsids_chrpos.bim $PWD/scripts/snp151_chr_pos_rsidsorted2.txt >  $PWD/$datasetname/$dataset/merged_nonrsids_chrpos_genotypes_snp151.txt
fi
...
plink2 --bfile $PWD/$datasetname/$dataset/updated_genotypes_fa_mapped_flipped --fa $PWD/scripts/hg19.fa --ref-from-fa force --make-bed --out $PWD/$datasetname/$dataset/genotypes_fa_flipped_fa
...
$PWD/scripts/liftOver -bedPlus=3 -tab $PWD/$datasetname/$dataset/genotypes_fa_flipped_fa_dropped.bim.hg18.bed $PWD/scripts/liftover/hg19ToHg38.over.chain.gz $PWD/$datasetname/$dataset/genotypes_fa_flipped_fa_dropped.bim.hg18.lifted_to_hg38.bed $PWD/$datasetname/$dataset/genotypes_fa_flipped_fa_dropped.bim.hg18.unlifted.bed
...
samtools faidx $PWD/scripts/hg38.fa -r $PWD/$datasetname/$dataset/genotypes_fa_flipped_fa_dropped.bim.hg18.lifted_to_hg38.bed.regions | grep -v "^>" > $PWD/$datasetname/$dataset/genotypes_fa_flipped_fa_dropped.bim.hg18.lifted_to_hg38.bed.regions.ref_alleles
